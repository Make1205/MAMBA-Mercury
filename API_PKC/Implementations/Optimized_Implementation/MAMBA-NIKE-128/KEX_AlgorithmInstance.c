...
typedef struct { unsigned char st_buf[sizeof(poly)]; unsigned char ma[4000]; unsigned long long ma_len; int ready; unsigned char tail_pad[2060]; } sta_t;
typedef struct { unsigned char mb[4000]; unsigned char ss[64]; unsigned long long mb_len; int ready; } stb_t;

static const nike_params* P(void){ return &NIKE_128; }
...
int kex_generate_pass1_msg_a(unsigned char *ska,unsigned long long ska_len_bytes,unsigned char *pkb,unsigned long long pkb_len_bytes,unsigned char *sta,unsigned long long *sta_len_bytes,unsigned char *m1,unsigned long long *m1_len_bytes){ (void)ska;(void)ska_len_bytes;(void)pkb;(void)pkb_len_bytes; sta_t*s=(sta_t*)sta; nike_state st; s->ma_len=nike_ma_bytes(P()); memset(&st,0,sizeof(st)); nike_init(&st,m1,P()); memset(s->st_buf,0,sizeof(s->st_buf));
  memcpy(s->st_buf,&st.s,sizeof(s->st_buf)); memcpy(s->ma,m1,s->ma_len); s->ready=1; *m1_len_bytes=s->ma_len; *sta_len_bytes=sizeof(sta_t); return 0; }
