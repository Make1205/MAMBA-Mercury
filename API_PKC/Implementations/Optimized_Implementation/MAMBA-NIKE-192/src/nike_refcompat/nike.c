...
static int nike_kdf(unsigned char *out,size_t outlen,const unsigned char *nu,size_t nulen,const unsigned char *ma,size_t malen,const unsigned char *mb,size_t mblen){
  ...
  while(done<outlen){ size_t off=0; memcpy(in+off,lbl,sizeof(lbl)-1); off+=sizeof(lbl)-1; in[off++]=outlen&0xff; in[off++]=(outlen>>8)&0xff; in[off++]=ctr&0xff; in[off++]=(ctr>>8)&0xff; in[off++]=(ctr>>16)&0xff; in[off++]=(ctr>>24)&0xff; memcpy(in+off,nu,nulen); off+=nulen; memcpy(in+off,ma,malen); off+=malen; memcpy(in+off,mb,mblen); off+=mblen; kdf_api_pkc(block,32,in,off); size_t take=(outlen-done<32)?(outlen-done):32; memcpy(out+done,block,take); done+=take; ctr++; }
  free(in); return 0;
}
